...
home_page = """ <html>
    ...
    <form method = post>
    <label>
    <textarea name = "data" rows=40 cols=120>%(content)s</textarea>
    </label>
    <div></div>
...
home_page_non_edit = """ <html>
    ...
    <form method = post>
    <label>
    <textarea readonly = True name = "data" rows=40 cols=120>%(content)s</textarea>
    </label>
    <div></div>
    </form> """
signup_page = """
...
class WikiPageHandler(WikHandler):
    ...
    def write_form_edit_link(self, edit_link, edit_link_text,  login_link, login_link_text ):
        self.response.out.write(edit_link_page % {'edit_link' : edit_link, 'login_link' : login_link,  'edit_link_text' : edit_link_text, 'login_link_text': login_link_text })
    ...
    def get(self):
        ...
        if not path:
            ...
            if not w:
            	...
            	w = Wiki.by_title('index_page_of_wiki')
            if self.user:
            	self.write_form_edit_link('http://' + self.request.url.split('/')[2] + '/_edit/index_page_of_wiki', 'edit','/logout','logout(' + self.user.user_name + ')')
            else:
            	self.write_form_edit_link('http://' + self.request.url.split('/')[2] + '/_edit/index_page_of_wiki', '','/login','login')
            self.response.out.write(HTMLParser.HTMLParser().unescape(w.content))
        else:
            w = Wiki.by_title(path)
            if w:
                if self.user:
	        	self.write_form_edit_link('http://' + self.request.url.split('/')[2] + '/' + '_edit/' + path, 'edit','/logout?page=' + path,'logout(' + self.user.user_name + ')')
	        else:
            		self.write_form_edit_link('http://' + self.request.url.split('/')[2] + '/' + '_edit/' + path, '','/login','login')
                self.response.out.write(HTMLParser.HTMLParser().unescape(w.content))
            else:
                if self.user:
                    if path[:5] == '_edit':
                        ...
                        if w:
                            self.write_form_edit_link('http://' + self.request.url.split('/')[2] +  '/' + path.split('/')[1], 'view','/logout?page=' + path.split('/')[1],'logout(' + self.user.user_name + ')' )
                            self.write_form(content=w.content)
                        else:
                            self.write_form_edit_link('http://' + self.request.url.split('/')[2] +  '/' + path.split('/')[1], '','/logout?page=' + path.split('/')[1],'logout(' + self.user.user_name + ')')
                            self.write_form()
                    else:
    ...
    def post(self):
        ...
        content = cgi.escape(self.request.get('data'), True)
        if w:
            if not content:
            	content=" "
